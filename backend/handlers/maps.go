...
func ListMaps(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		const q = `SELECT id, name, created_at FROM maps WHERE organization_id = $1 ORDER BY created_at`
...
func CreateMap(pool *pgxpool.Pool) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		var m gameMap
		const q = `
			INSERT INTO maps (organization_id, name)
			VALUES ($1, $2)
			RETURNING id, name, created_at`
		if err := pool.QueryRow(r.Context(), q, orgID, name).Scan(&m.ID, &m.Name, &m.CreatedAt); err != nil {
